...
export function TextboxSimple({ type, label, value, setValue, size, mode, placeholder, icon, width, onChange, important }) {
    ...
    return (
        ...
        <div className={`${width || "max-w-md"} mx-auto`}>
            ...
            <div className={`relative ${getHeight(sizeState)} bg-white flex border-[1.5px] border-gray-400 rounded-[4px] items-center`}>
                <input type={type || "text"} value={value} className={`pl-2 appearance-none outline-none text-gray-800 w-full ${icon ? "pr-7" : "pr-2"}`}
